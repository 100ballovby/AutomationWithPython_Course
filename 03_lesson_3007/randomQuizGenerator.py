...
for quizNum in range(35):
    quizFile = open(os.path.join('exam', 'questions', f'cap_quiz_{quizNum + 1}.txt'), 'w')
    answerKey = open(os.path.join('exam', 'answer', f'ans_quiz_{quizNum + 1}.txt'), 'w')
    # создаю файлы с вопросами и ключами-ответами

    quizFile.writelines('Name:\n\nDate:\n\nForm\n\n')  # заполняю шапку билета
    quizFile.writelines((' ' * 20) + f'State + capitals quiz. Question №{quizNum + 1}\n\n')  # записываю номер билета

    states = list(capitals.keys())  # ключи словаря превращаю в список штатов
    random.shuffle(states)  # перемешиваю список

    for questionNum in range(50):  # для всех 50 штатов
        correct_answer = capitals[states[questionNum]]  # достану ПРАВИЛЬНУЮ столицу штата
    ...
    # записываем правильные ответы в файл
    answerKey.write(f'{questionNum + 1}. {"1234"[ans_option.index(correct_answer)]}\n')
